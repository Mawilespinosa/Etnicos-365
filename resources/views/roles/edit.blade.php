...
@section('content')
    ...
    <form method="POST" action="{{ route('roles.update', $role) }}" class="bg-white rounded shadow p-6 max-w-3xl">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input id="display_name" type="text" name="display_name" value="{{ old('display_name', $role->display_name) }}" required
                   class="w-full rounded border border-gray-300 px-3 py-2">
            @error('display_name')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
                        ...
                        @foreach ($modulePermissions as $permission)
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->id }}"
                                       @checked(in_array($permission->id, old('permissions', $rolePermissions))) class="rounded">
