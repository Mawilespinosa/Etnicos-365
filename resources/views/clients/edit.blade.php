...
@section('content')
    ...
    <form method="POST" action="{{ route('clients.update', $client) }}" class="bg-white rounded shadow p-6 max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="mb-4">
                ...
                <select id="document_type" name="document_type" required class="w-full rounded border border-gray-300 px-3 py-2">
                    <option value="CC" @selected(old('document_type', $client->document_type) === 'CC')>Cédula de ciudadanía (CC)</option>
                    <option value="CE" @selected(old('document_type', $client->document_type) === 'CE')>Cédula de extranjería (CE)</option>
                    <option value="NIT" @selected(old('document_type', $client->document_type) === 'NIT')>NIT</option>
                </select>
                @error('document_type')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input id="document_number" type="text" name="document_number" value="{{ old('document_number', $client->document_number) }}" required
                       class="w-full rounded border border-gray-300 px-3 py-2">
        ...
        <div class="mb-6">
            <label class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" value="1" @checked($client->is_active) class="rounded">
