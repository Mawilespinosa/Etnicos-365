...
@section('content')
    <h1 class="text-2xl font-bold mb-6">Editar orden {{ $order->code }}</h1>

    <form method="POST" action="{{ route('production.orders.update', $order) }}" class="bg-white rounded shadow p-6 max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input id="quantity" type="number" step="0.01" min="0.01" name="quantity"
        ...
        <div class="mb-6">
            ...
            <textarea id="notes" name="notes" rows="3"
                      class="w-full rounded border border-gray-300 px-3 py-2">{{ old('notes', $order->notes) }}</textarea>
            @error('notes')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('production.orders.show', $order) }}" class="text-gray-600 hover:underline">Cancelar</a>
        </div>
    </form>
@endsection