...
@section('content')
    ...
    <form method="POST" action="{{ route('products.update', $product) }}" class="bg-white rounded shadow p-6 max-w-2xl" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea id="description" name="description" rows="3"
                      class="w-full rounded border border-gray-300 px-3 py-2">{{ old('description', $product->description) }}</textarea>
            @error('description')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="mb-4">
                ...
                <input id="size" type="text" name="size" value="{{ old('size', $product->size) }}"
                       class="w-full rounded border border-gray-300 px-3 py-2">
                @error('size')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="mb-4">
                ...
                <input id="model" type="text" name="model" value="{{ old('model', $product->model) }}"
                       class="w-full rounded border border-gray-300 px-3 py-2">
                @error('model')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="mb-4">
                ...
                <input id="cost" type="number" step="0.01" min="0" name="cost" value="{{ old('cost', $product->cost) }}" required
                       class="w-full rounded border border-gray-300 px-3 py-2">
                @error('cost')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="mb-4">
                ...
                <input id="stock_qty" type="number" step="0.01" min="0" name="stock_qty" value="{{ old('stock_qty', $product->stock_qty) }}"
                       class="w-full rounded border border-gray-300 px-3 py-2">
                @error('stock_qty')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input id="min_stock" type="number" step="0.01" min="0" name="min_stock" value="{{ old('min_stock', $product->min_stock) }}"
                       class="w-full rounded border border-gray-300 px-3 py-2">
                @error('min_stock')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-4">
            ...
            @if ($product->image)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="max-h-32 rounded border border-gray-200">
                </div>
            @else
        ...
        <div class="mb-4">
            <label for="image" class="block text-sm font-medium mb-1">Cambiar imagen</label>
            <input id="image" type="file" name="image" accept="image/jpeg,image/png,image/webp"
                   class="w-full rounded border border-gray-300 px-3 py-2">
            @error('image')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
            <p class="text-xs text-gray-500 mt-1">Formatos permitidos: JPG, PNG, WebP. Máximo 2 MB. Dejar vacío para mantener la imagen actual.</p>
        </div>

        <div class="mb-6">
            <label class="flex items-center gap-2">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" name="is_active" value="1" @checked($product->is_active) class="rounded">
