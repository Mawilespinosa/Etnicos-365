...
@section('content')
    ...
    <form method="POST" action="{{ route('users.update', $user) }}" class="bg-white rounded shadow p-6 max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            <label for="password" class="block text-sm font-medium mb-1">Nueva contraseña <span class="text-gray-400">(opcional)</span></label>
                ...
                @foreach ($roles as $role)
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="roles[]" value="{{ $role->id }}"
                               @checked(in_array($role->id, old('roles', $user->roles->pluck('id')->all()))) class="rounded">
